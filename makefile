...
PPSOURCES =  bondpp.C bondpp.h matrixroutines.h overload.h bravaislattices.h vecmat.h modeldef.h couplings.h modelcouplings.h rules.h phonons.h inputparameters.h symmetryroutines.h observables.h makefile Makefile.local globalheader.h RunParameter.h rnddef.h
...
squareHeisenberg_rev.o  : $(SQUARESOURCES)
	$(CCC) $(CCFLAGS) -DSIMPLECUBICBRAVAISLATTICE -DNSUBLATTICES=1 -DSQUAREPHONONS -DSQUARELATTICE  -DQSPACEDIRECTLATTICE -DPHONONS -DXYDISPLACEMENTS -DUSELASTSIGMA -DREVERSEDELTALIST -DFORCEINVERSIONSYMMETRY -DFAKEHEISENBERG -DCPOSITIVE -DNBRRANGE=2 -DFFTS_INPLACE -c -o $@ $<


squareHeisenbergnophonons.o  : $(SQUARESOURCES)
	$(CCC) $(CCFLAGS) -DSIMPLECUBICBRAVAISLATTICE -DNSUBLATTICES=1 -DSQUAREPHONONS -DSQUARELATTICE  -DQSPACEDIRECTLATTICE -DPHONON#S -DXYDISPLACEMENTS -DRANDOMINITIALIZATION -DFORCEINVERSIONSYMMETRY -DFAKEHEISENBERG -DCPOSITIVE -DNBRRANGE=2 -DFFTS_INPLACE -DOMITPHONONCONT -DOMITELASTICCONT -c -o $@ $<
