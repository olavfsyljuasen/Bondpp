...
public:
  ...
  int GetN(){return N;}
  int GetNR(){return NR;}
  realtype GetPar(const int r,const int i){return param[r-1][i];}
  ...
  realtype* GetPars(const int r){return &param[r-1][0];}
private:
  int NR;
  int N;
  string file;
  realtype param[MAXRUNS][MAXPAR];
};  
...
RunParameters::RunParameters(string name): 
  ...
  if(!ifile){ logfile << "cound not find " << file << endl;}
  for(int r = 0; r < NR; r++)
